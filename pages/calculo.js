import React from 'react'
import { useState } from 'react';
import {
  Button, Collapse, AppShell,
  Container,
  NativeSelect,
  Stack
} from '@mantine/core';
import Menu from '../components/Menu';
import Harris from './paginas/harris';
import Oms from './paginas/oms';
import Owen from './paginas/owen';
import Valencia from './paginas/valencia';
import Mifflin from './paginas/mifflin';
import Cuadrodieta from './cuadrodieta';
import Layout from '../components/Layout';

const Calculo = () => {

  const [opened, setOpened] = useState(false);
  const [opened2, setOpened2] = useState(false);
  const [opened3, setOpened3] = useState(false);
  const [opened4, setOpened4] = useState(false);
  const [opened5, setOpened5] = useState(false);
  const [formula, setFormula] = useState(0.00)

  const [kcalTotal, setKcalTotal] = useState(0);
  const [peso, setPeso] = useState(0);

  const guardarKcalTotal = (kcal) => {
    setKcalTotal(kcal);
  }

  const guardarPeso = (inputPeso) => {
    setPeso(inputPeso);
  }


  const abrirTab = (tab) => {
    console.log(tab);
    setFormula(tab);
    cerrarTodos();
    switch (tab) {
      case 'Harris-benedict':
          setOpened(true);
        break;
      case 'Oms':
          setOpened2(true);
        break;
      case 'Owen':
          setOpened3(true);
        break;
      case 'Valencia':
          setOpened4(true);
        break; 
      case 'Mifflin St-Jeor':
          setOpened5(true);
        break;

          default:
        break;
    }
  }
  const cerrarTodos = () => {
    setOpened(false);
    setOpened2(false);
    setOpened3(false);
    setOpened4(false);
    setOpened5(false);
  }

  return (
    <Layout isPrivate = {true}>
      <AppShell
        navbarOffsetBreakpoint="sm"
        asideOffsetBreakpoint="sm"
        navbar={
          <Menu></Menu>
        }
      >
        <Stack justify="flex-start" spacing="xl" >
          <h1>Calculo Dietetico </h1>

          <Container size="sm" px="sm">
            <NativeSelect
              data={['Seleccionar','Harris-benedict', 'Oms', 'Owen', 'Valencia', 'Mifflin St-Jeor']}
              value={formula}
              placeholder="Selecciona uno"
              label="Selecciona la formula a calcular."
              onChange={(event) => abrirTab(event.currentTarget.value)}
              withAsterisk
            />
          </Container>


          <Button color="violet" onClick={() => setOpened((o) => !o)} variant="subtle"> Harris</Button>
          <Collapse in={opened}  className="">
            <Harris onKcal={guardarKcalTotal} onPeso={guardarPeso}> </Harris>
          </Collapse>
          <Button color="violet" onClick={() => setOpened2((o) => !o)} variant="subtle">OMS</Button>
          <Collapse in={opened2}>
            <Oms></Oms>
          </Collapse>
          <Button color="violet" onClick={() => setOpened3((o) => !o)} variant="subtle">Owen</Button>
          <Collapse in={opened3}>
            <Owen></Owen>
          </Collapse>
          <Button color="violet" onClick={() => setOpened4((o) => !o)} variant="subtle">Valencia</Button>
          <Collapse in={opened4}>
            <Valencia></Valencia>
          </Collapse>
          <Button color="violet" onClick={() => setOpened5((o) => !o)} variant="subtle">Mifflin ST-Jeor</Button>
          <Collapse in={opened5}>
            <Mifflin></Mifflin>
          </Collapse>

          {kcalTotal == 0 ? <div></div> : <Cuadrodieta activador= {0} kcal={kcalTotal} peso={peso} />}

        </Stack>

      </AppShell>
    </Layout>
  )
}


export default Calculo